...
df = df.drop(columns=['#','BOROUGH','BLOCK', 'YEAR BUILT', 'current_year', 'TOTAL UNITS', 'LAND SQUARE FEET'])
...
df['SALE PRICE'] = df['SALE PRICE'][np.abs(df['SALE PRICE']-df['SALE PRICE'].mean()) <= (.25*df['SALE PRICE'].std())]
